...
#ifndef HAL_BATTMON_TIBQ76952_BUS
#define HAL_BATTMON_TIBQ76952_BUS 0
#endif
#ifndef HAL_BATTMON_TIBQ76952_ADDR
#define HAL_BATTMON_TIBQ76952_ADDR 0
#endif
...
public:
    ...
    bool has_max_current() const override { return true; }
    bool get_max_current(float &max_current) const override;
    ...
    void set_discharge(bool enable) override;
    
    // Get power state
    AP_BattMonitor::PowerState get_state() const { return _state.power_state; }
...
protected:
    ...
    uint8_t direct_command_8bit(uint16_t reg) const;
    uint16_t direct_command_16bit(uint16_t reg) const;
    uint32_t sub_command_32bit(uint16_t reg) const;
    uint16_t read_voltage(uint8_t command) const;
    void safety_status_update(void);
    void alarm_raw_status_update(void);
    ...
    struct {
        ...
        float max_current;  // maximum current observed in amps
        float temp;         // battery temperature in degrees Celsius
        float health_pct;   // battery health percentage
        bool is_powering_off; // battery is powering off
        mutable HAL_Semaphore sem;
    } accumulate;

    // State machine variables
    AP_BattMonitor::PowerState previous_state;
    uint32_t state_change_ms;       // timestamp of last state change
    uint32_t time_in_state_ms;      // time spent in current state
    float current_threshold_idle;   // current threshold to consider battery idle (A)
    float current_hysteresis;       // hysteresis to prevent state flapping (A)
