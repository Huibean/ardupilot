...
#define CRC_MODE 0  // BQ76952 requires CRC for reliable SPI communication
#define DISABLE_TS1
#define DISABLE_TS3
...
{
    _dev = hal.i2c_mgr->get_device(HAL_BATTMON_TIBQ76952_BUS, HAL_BATTMON_TIBQ76952_ADDR);
    if (!_dev) {
        return;
    }
    dev = _dev.get();
...
{
    // configure device on first call
    if (!configured) {
        if (!configure()) {
            return;
        }
...
{
    // with 5V supply from the can bus, without correct OTP BQ76952 reg won't power AP_Periph 
    // sub_command(TIBQ769x2_RESET);
    // hal.scheduler->delay_microseconds(10000);

    // check device id, exit on failure
    device_number = sub_command_32bit(TIBQ769x2_DEVICE_NUMBER);
    ...
    fw_version = sub_command_32bit(TIBQ769x2_FW_VERSION);
    hw_version = sub_command_32bit(TIBQ769x2_HW_VERSION);

    // debug
    printf("BQ76952 detected, fw: 0x%08lX, hw: 0x%08lX\n", (unsigned long)fw_version, (unsigned long)hw_version);

    //enable REG1 3.3v to provide power to the MCU 
    set_register(TIBQ769x2_REG12Config, 0x01, 1); // 0x01 for 3.3V output
    ...
    configured = true;
    printf("BQ76952: Configuration complete\n");
...
void AP_BattMonitor_TIBQ76952::read(void)
{
    ...
    */
    ...
    _state.current_amps = fabsf(accumulate.current); 
    _state.max_current_amps = accumulate.max_current;
...
bool AP_BattMonitor_TIBQ76952::get_max_current(float &max_current) const
{
    WITH_SEMAPHORE(accumulate.sem);
    if (accumulate.count == 0) {
        return false;
    }
    max_current = accumulate.max_current;
    return true;
}
...
bool AP_BattMonitor_TIBQ76952::read_register(uint8_t reg_addr, uint8_t *reg_data, uint8_t count) const
{
    ...
    // For I2C, just read directly
    return dev->read_registers(reg_addr, reg_data, count);
}
...
uint8_t AP_BattMonitor_TIBQ76952::direct_command_8bit(uint16_t reg) const
{
    uint8_t rx_data[1];
    if (direct_command(reg, 0x00, READ, rx_data, 1)) {
        return rx_data[0];
    }
    return 0;
}

uint16_t AP_BattMonitor_TIBQ76952::direct_command_16bit(uint16_t reg) const
{
    uint8_t rx_data[2];
    if (direct_command(reg, 0x00, READ, rx_data, 2)) {
        return rx_data[1] << 8 | rx_data[0];
    }
    return 0;
}

uint32_t AP_BattMonitor_TIBQ76952::sub_command_32bit(uint16_t reg) const
{
    uint8_t rx_data[4];
    if (sub_command(reg, 0x00, READ, rx_data, 4)) {
...
uint16_t AP_BattMonitor_TIBQ76952::read_voltage(uint8_t command) const
{
    ...
    uint16_t voltage = direct_command_16bit(command);
    if (voltage) {
...
void AP_BattMonitor_TIBQ76952::safety_status_update(void)
{
    uint16_t safetyA_status = direct_command_16bit(TIBQ769x2_SafetyStatusA); 
    if (safetyA_status > 0) {
        printf("BQ76952: SafetyA Status: 0x%04X\n", safetyA_status);
    }
}

void AP_BattMonitor_TIBQ76952::alarm_raw_status_update(void)
{
    uint16_t alarm_raw_status = direct_command_16bit(TIBQ769x2_AlarmRawStatus); 

    //FULL_SCAN bit 7 
    if (alarm_raw_status & 0x0080) {
        // read voltage
        WITH_SEMAPHORE(accumulate.sem);
        ...
        for (uint8_t i = 0; i < CELL_COUNT; i++) {
            cell_voltage_mv[i] = direct_command_16bit(TIBQ769x2_Cell1Voltage + i*2);
        ...
        // Read current (signed value: positive = discharge, negative = charge)
        int16_t cc2_current = direct_command_16bit(TIBQ769x2_CC2Current); 
        accumulate.current = cc2_current * 0.001f; // Convert to Amps

        // Track maximum current only during discharge
        if (_state.power_state == AP_BattMonitor::PowerState::DISCHARGING) {
            accumulate.max_current = MAX(accumulate.max_current, accumulate.current);
        }

        int16_t temp_internal = direct_command_16bit(TIBQ769x2_IntTemperature); // 0.1K
        accumulate.temp = KELVIN_TO_C(temp_internal * 0.1f); // Convert to °C
...
// Control discharge FET
void AP_BattMonitor_TIBQ76952::set_discharge(bool enable)
{
    if (!configured) {
        return;
    }
...
// Update battery state based on current flow
void AP_BattMonitor_TIBQ76952::battery_state_update()
{
    // const uint32_t now_ms = AP_HAL::millis();
    AP_BattMonitor::PowerState new_state = _state.power_state;

    uint16_t mfg_status = direct_command_16bit(TIBQ769x2_MfgStatusInit);
    uint8_t fet_status = direct_command_8bit(TIBQ769x2_FETStatus);
    // printf("BQ76952: Mfg Status: 0x%04X, FET Status: 0x%02X\n", mfg_status, fet_status);

    if (mfg_status & MfgStatusInit_FET_EN) {
        new_state = AP_BattMonitor::PowerState::IDLE;
        if (fet_status & FET_STATUS_CHG_FET_EN && accumulate.current > 0) {
            // Charging if CHG_FET bit is set
            new_state = AP_BattMonitor::PowerState::CHARGING;
        }
        if (fet_status & FET_STATUS_DSG_FET_EN) {
            // Discharging if DSG_FET bit is set
            new_state = AP_BattMonitor::PowerState::DISCHARGING;
        }  
    }
    // printf("BQ76952: Battery State: %s\n",
    //        (new_state == AP_BattMonitor::PowerState::CHARGING) ? "CHARGING" :
    //        (new_state == AP_BattMonitor::PowerState::DISCHARGING) ? "DISCHARGING" :
    //        (new_state == AP_BattMonitor::PowerState::IDLE) ? "IDLE" : "UNKNOWN");

    _state.power_state = new_state;
